...
def test_readiness_500_db_invalid():
    ...
    with mock.patch('django.db.backends.utils.CursorWrapper',
                    mock.MagicMock(side_effect=lambda x, y: cursor)):
        response = Client().get('/readiness/')
...
def test_readiness_500_cache_conn_err():
    def mock_all(x):
        raise x
    with mock.patch('django.core.cache.CacheHandler.all',
                    mock.MagicMock(
                        side_effect=lambda initialized_only: [] if initialized_only else
                        mock_all(Exception()))):
        response = Client().get('/readiness/')
        assert response.status_code == 500
